...
// use chess_model::chess_move::move_parser;
// use log::info;
...
//   // let _ = DataPacketToServer::commit_move(0, move_parser::parse_move_str("p2~5", chess_model::piece::PieceColor::RED).unwrap()).send(&mut stream);
//   let _ = DataPacketToServer::request_fail(0).send(&mut stream);
...
//   sleep(Duration::from_secs(3));
//   let _ = DataPacketToServer::agree_fail(0, false).send(&mut stream);
